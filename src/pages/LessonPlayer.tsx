...
import { useLessonNeighbors } from "@/hooks/useLessonNeighbors";
import remarkGfm from "remark-gfm";

const MarkdownBlock = ({ content }: { content: string }) => (
  <div className="prose prose-invert max-w-none break-words prose-headings:text-foreground prose-p:text-muted-foreground prose-strong:text-foreground prose-code:rounded prose-code:bg-muted prose-code:px-1.5 prose-code:py-0.5 prose-code:text-primary prose-code:break-words prose-a:text-primary prose-a:break-all prose-pre:overflow-x-auto prose-pre:max-w-[calc(100vw-2rem)] prose-img:max-w-full">
...
const LessonPlayer = () => {
  ...
  // neighbors hook must be called unconditionally to preserve hook order
  const { data: neighbors } = useLessonNeighbors(lessonId);
  ...
  const prevLesson = neighbors?.prev;
  const nextLesson = neighbors?.next;
  const canGoNext = Boolean(nextLesson && isCompleted);
  ...
  const renderBlock = (block: any) => {
    ...
    switch (block.block_type) {
      case "markdown":
        return <MarkdownBlock content={c.markdown || c.content || ""} />;
      case "video":
        return <VideoBlock url={c.url || ""} title={c.title} />;
      case "code":
        return <CodeBlock code={c.code || ""} language={c.language} />;
      case "resources":
        return <ResourcesBlock links={c.links || []} />;
      case "checklist":
        return <ChecklistBlock items={c.items || []} />;
      default:
        return (
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8 w-full min-w-0">
      <Button
        variant="ghost"
        size="sm"
        className="mb-6"

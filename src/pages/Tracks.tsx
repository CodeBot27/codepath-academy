...
const TrackSection = ({ track }: { track: { id: string; title: string; description?: string } }) => {
  const { data: courses, isLoading } = useCoursesByTrack(track.id);
  const hasContent = !isLoading && courses && courses.length > 0;
  const isComingSoon = !isLoading && !hasContent;
    ...
    <section className="mb-12">
      ...
      {isLoading ? (
        ...
        <div className="rounded-lg border border-dashed border-border bg-muted/30 p-8 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground/70">Check back soon for new courses in this track.</p>
        </div>
      ) : (
